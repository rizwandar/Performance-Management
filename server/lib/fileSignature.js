// A client can freely lie about a file's MIME type and extension, so the
// upload routes' allowlist check alone isn't enough - this verifies the
// uploaded bytes actually look like the file type they claim to be, using
// each format's magic-byte signature (SEC-11).
//
// Deliberately dispatched via an explicit switch on string literals, not a
// lookup table (object/Map) keyed by the caller-supplied extension - a
// table-based dispatch, even one guarded by hasOwnProperty or backed by a
// prototype-less Map, still has the shape "call a function selected by a
// tainted key," which CodeQL's unvalidated-dynamic-method-call query flags
// regardless of how safe the table itself is. A switch over literals has no
// dynamic dispatch at all: every branch calls one specific, hardcoded
// function, so there is no function whose identity could ever be influenced
// by the input.

function isPdf(buf) {
  return buf.subarray(0, 5).toString('latin1') === '%PDF-';
}

function isJpg(buf) {
  return buf.length >= 3 && buf[0] === 0xFF && buf[1] === 0xD8 && buf[2] === 0xFF;
}

function isPng(buf) {
  return buf.length >= 8 &&
    buf.subarray(0, 8).equals(Buffer.from([0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A]));
}

function isWebp(buf) {
  return buf.length >= 12 &&
    buf.subarray(0, 4).toString('latin1') === 'RIFF' &&
    buf.subarray(8, 12).toString('latin1') === 'WEBP';
}

// HEIC is ISOBMFF: a size field, then an 'ftyp' box whose brand identifies
// the encoding. Apple's HEIC photos use a handful of brand codes.
function isHeic(buf) {
  if (buf.length < 12 || buf.subarray(4, 8).toString('latin1') !== 'ftyp') return false;
  const brand = buf.subarray(8, 12).toString('latin1');
  return brand.startsWith('hei') || brand.startsWith('hev') || brand === 'mif1' || brand === 'msf1';
}

// Legacy binary .doc is an OLE compound file.
function isDoc(buf) {
  return buf.length >= 8 &&
    buf.subarray(0, 8).equals(Buffer.from([0xD0, 0xCF, 0x11, 0xE0, 0xA1, 0xB1, 0x1A, 0xE1]));
}

// .docx is a zip. Checking for the "word/" internal entry path (rather than
// just the zip signature) rejects a renamed .xlsx/.pptx or a plain zip file
// without needing a full zip-parsing dependency.
function isDocx(buf) {
  return buf.length >= 4 &&
    buf.subarray(0, 4).toString('latin1') === 'PK\x03\x04' &&
    buf.includes('word/');
}

// SVG is plain-text XML, not a binary format with a fixed byte signature, so
// this checks for a leading XML/svg declaration instead.
function isSvg(buf) {
  const text = buf.subarray(0, 256).toString('utf8').replace(new RegExp('^﻿'), '').trimStart();
  return /^<\?xml/i.test(text) || /^<svg[\s>]/i.test(text);
}

// IDEA-01 voice messages. WebM/Matroska share the same EBML container header
// regardless of which codec is inside.
function isWebm(buf) {
  return buf.length >= 4 && buf.subarray(0, 4).equals(Buffer.from([0x1A, 0x45, 0xDF, 0xA3]));
}

function isOgg(buf) {
  return buf.length >= 4 && buf.subarray(0, 4).toString('latin1') === 'OggS';
}

// mp4/m4a are both the ISOBMFF container (same 'ftyp' box as HEIC above,
// just checking for the box's presence rather than a specific brand code -
// audio/video mp4 brands vary too widely to enumerate usefully here).
function isMp4(buf) {
  return buf.length >= 8 && buf.subarray(4, 8).toString('latin1') === 'ftyp';
}

function isWav(buf) {
  return buf.length >= 12 &&
    buf.subarray(0, 4).toString('latin1') === 'RIFF' &&
    buf.subarray(8, 12).toString('latin1') === 'WAVE';
}

// MP3: either a leading ID3 tag, or a raw MPEG frame sync (11 set bits).
function isMp3(buf) {
  return buf.length >= 3 && (
    buf.subarray(0, 3).toString('latin1') === 'ID3' ||
    (buf[0] === 0xFF && (buf[1] & 0xE0) === 0xE0)
  );
}

function matchesExtension(buffer, ext) {
  switch (ext?.toLowerCase()) {
    case 'pdf':  return isPdf(buffer);
    case 'jpg':
    case 'jpeg': return isJpg(buffer);
    case 'png':  return isPng(buffer);
    case 'webp': return isWebp(buffer);
    case 'heic': return isHeic(buffer);
    case 'doc':  return isDoc(buffer);
    case 'docx': return isDocx(buffer);
    case 'svg':  return isSvg(buffer);
    case 'webm': return isWebm(buffer);
    case 'ogg':  return isOgg(buffer);
    case 'mp4':
    case 'm4a':  return isMp4(buffer);
    case 'wav':  return isWav(buffer);
    case 'mp3':  return isMp3(buffer);
    default:     return false;
  }
}

module.exports = { matchesExtension };
