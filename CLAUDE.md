...
- **Known gap, tracked separately:** dev/staging/production currently share one Cloudflare R2 bucket (and staging/production share R2 credentials too) - not yet split into separate buckets/credentials per environment. See the Security tab in the admin panel.
