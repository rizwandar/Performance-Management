    ...
    <div style={{ maxWidth: 820, margin: '0 auto' }}>
      ...
      {section('vault', '3. The Digital Vault', <>
        ...
        {li([
          ...
          'Recovery via security questions (optional): you set up 3-5 vault-specific security questions. If you forget your password, answering at least 3 of them correctly recovers access without deleting anything. Choosing this option means your vault confidentiality is only as strong as those answers, not "even we cannot read it" - a database breach combined with guessable or known answers could expose your vault data. We never store the answers themselves, only encrypted material that can only be unlocked by supplying at least 3 correct answers.',
        ])}
      ...
      {section('vaultattempts', '4. Vault Attempt Protection', <>
        {p('Failed vault password attempts are tracked per account. Each failed attempt triggers a security email so you know if someone is trying to access your vault. After 5 consecutive failed attempts you are signed out and must log in again before trying more. Every 5 failed attempts, the vault is temporarily locked for 3 minutes as protection against sustained guessing. A correct password always unlocks the vault immediately, even mid-lockout, and resets the attempt count to zero.')}
        {p('Incorrect attempts do not delete anything. Permanent deletion after repeated wrong attempts is an optional "maximum security" setting that is switched off for every account unless you turn it on yourself in Profile > Vault Settings, after an explicit warning. If you do turn it on, you choose the threshold (between 3 and 1000 consecutive wrong attempts), and reaching it permanently deletes all vault-protected data with no way for you or for us to recover it. You can switch it back off at any time.')}
      </>)}
