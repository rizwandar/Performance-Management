      ...
      <div style={card}>
        <BpSection title="7. Authentication & Security">
          <BpTable rows={[
            ...
            ['CORS', 'Manual CORS implementation in server/index.js. An Origin is echoed back with Access-Control-Allow-Credentials only when it exactly matches the CLIENT_URL env var. If CLIENT_URL is unset, reflection of an arbitrary Origin is confined to a positively-identified local development machine (the same isLocalDevelopment() check lib/jwtSecret.js uses); a deployed environment missing CLIENT_URL sends no CORS headers at all rather than opening to every origin. It previously reflected any Origin whenever CLIENT_URL was unset, on any environment.'],
            ['Admin account', 'Seeded on first run only (skipped if an admin@igh.local row already exists): email="admin@igh.local", is_admin=1 on the users table. The password is random per database and is never disclosed. Access is bootstrapped through a single-use password reset link valid 7 days, emailed to ADMIN_SEED_NOTIFY_EMAIL (falling back to ADMIN_EMAIL) and also written to the server log in case Resend is not configured. It was previously the fixed string "Admin1234", which shipped to every environment and is published in this repository history.'],
