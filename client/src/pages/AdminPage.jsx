      ...
      <div style={card}>
        <BpSection title="4. Database Schema (PostgreSQL)">
          ...
          {[
            {
              table: 'users',
              fields: 'id, name, email (unique), password_hash, date_of_birth, life_story, about_me, remembered_for, legacy_message, emergency_contact_name, emergency_contact_phone, emergency_contact_email, marital_status, spouse_name, spouse_phone, spouse_email, is_admin (0/1), inactivity_period_months (default 12), last_active_at, reset_token (SHA-256 hash, not the raw token), reset_token_expiry, session_version (bumped on password change/reset, invalidates older JWTs), security_question (plain text, not sensitive), security_answer_hash (bcrypt, never returned to the client), must_set_security_question (1 for accounts registered after FEAT-06, drives a one-time blocking onboarding step until a question is set; existing accounts default to 0 and keep the older non-blocking nudge instead), vault_attempts, vault_locked_until (NULL = not locked, future timestamp = locked until then, replaces the old delete-on-5th-attempt behaviour), created_at, plus email verification and privacy consent fields',
            },
            {
